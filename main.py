...
def process_settlement(id_code, action, value=None):
    ...
    row = df[df['행동'] == action]
    if not row.empty:
        return row.iloc[0].get('Unnamed: 3', '정산 처리 완료')
    return '정산 처리 완료'
...
def skill():
    ...
    type_ = params.get('type')  # 'auth', 'investigate', 'settle', 'random', 'investigate_tree'
    ...
    if type_ == 'auth':
        ...
        if not user:
            ...
            response = user.get('출력지문', f"{user['Name']}님 어서오세요.")
